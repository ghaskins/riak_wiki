...
{info:title=Message Definitions}
All Protocol Buffers messages can be found defined in the [[riakclient.proto|http://github.com/basho/riak-erlang-client/blob/master/src/riakclient.proto]] file in the Erlang client.
{info}
...
{note}
The PBC interface does not currently support all bucket properties. It is currently limited to {{allow_mult}} and {{n_val}}; other bucket properties would need to be set with the [[REST API|REST API#Set bucket properties]].
{note}
...
Optional Parameters:
* {{r}} \- (read quorum) how many replicas need to agree when retrieving the object; possible values include 'default', 'one', 'quorum', 'all', or any integer <= N ([[default is defined per the bucket|#Set Bucket Properties]])
...
Each object can contain user-supplied metadata (X-Riak-Meta-\* in the HTTP interface) consisting
of a key/value pair. e.g. key=X-Riak-Meta-ACL value=users:r,administrators:f would allow an application to store access control information for it to enforce (*not* Riak).
...
* {{w}} \- (write quorum) how many replicas to write to before returning a successful response; possible values include 'default', 'one', 'quorum', 'all', or any integer <= N ([[default is defined per the bucket|#Set Bucket Properties]])
* {{dw}} \- how many replicas to commit to durable storage before returning a successful response; possible values include 'default', 'one', 'quorum', 'all', or any integer <= N ([[default is defined per the bucket|#Set Bucket Properties]])
...
vclock: "k\\316a```\\312`\\312\\005R,,\\351\\014\\206\\031L\\211\\214y\\254\\014Z!\\266G\\371 \\302l\\315I\\254\\rw|\\240\\022\\372 \\211,\\000"
...
Optional Parameters:
* {{rw}} \- how many replicas to delete before returning a successful response; possible values include 'default', 'one', 'quorum', 'all', or any integer <= N ([[default is defined per the bucket|#Set Bucket Properties]])
