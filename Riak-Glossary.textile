...
A Bucket is a container and [[keyspace|#Key]] for data stored in Riak, with a set of common properties for its contents (the number of [[replicas|#Replica]], for instance).  Buckets are accessed at the top of the URL hierarchy under "riak", e.g. {{/riak/bucket}}.
* [[Take a more in depth look at Bucket Operations|RIAK:REST API#Bucket Operation]]

h5. Consistent Hashing

Consistent hashing is a technique used to limit the reshuffling of keys when a hash-table data structure is rebalanced (when slots are added or removed). Riak uses consistent hashing to organize its data storage and replication. A Riak cluster is a 160-bit integer space which is divided into equally-sized partitions. Each [[vnode|#Vnodes]] in the [[Riak ring|#Ring]] is responsible for one of these partitions, and the location on the ring responsible for storing each object in Riak is determined using the consistent hashing technique.
...
Keys are unique object identifiers in Riak and are scoped within [[buckets|#Bucket]].
...
A node is analogous to a physical server. Nodes run a certain number of [[vnodes|#Vnode]], which in turn claim a set number of [[partitions|#partition]] in the Riak [[ring|#Ring]] key space.
...
* [[More about Read Repair|Replication#Read Repair]]
...
The Riak Ring is a 160-bit integer space. This space is equally divided into [[partitions|#Partition]], which are claimed by [[vnodes|#Vnode]], which reside on actual physical server [[nodes|#Node]].
...
* [[Read more on Vector Clocks|http://en.wikipedia.org/wiki/Vector_clock]]
* [Blog post "Why Vector Clocks are Easy"|http://blog.basho.com/2010/01/29/why-vector-clocks-are-easy/]

h5. Vnode

Vnodes, or "virtual nodes" are responsible for claiming a [[partition|#Partition]] in the Riak [[Ring|#Ring]], and they coordinate requests for these partitions. Vnodes map to physical [[nodes|#Node]] in a Riak cluster, and the number of vnodes on each physical node is determined by the number of active physical nodes in the cluster.
