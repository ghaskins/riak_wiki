...
The essential way to read and modify links via the [[REST API]] is the [[HTTP Link header|http://www.martinpayne.me.uk/experiments/HTTP/Link-Header/HTTP-Link-example.html]].  This header emulates the purpose of {{<link>}} tags in HTML, that is, establishing relationships to other HTTP resources.  The format that Riak uses is like so:
...
To access the links, use [{{riak_object:get_metadata/1}}|http://riak.basho.com/edoc/riak_object.html#get_metadata-1] to retrieve the metadata {{dict}}, and then retrieve the {{<<"Links">>}} key from that dict.  Example:
