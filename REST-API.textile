...
{note:title=Client ID}
All requests should include the {{X-Riak-ClientId}} header, which can be any string that uniquely identifies the client, for purposes of tracing object modifications in the [[vector clock|Riak Glossary#Vector Clock]].
{note}

{note:title=URL Escaping}Buckets, keys, and link specifications may not contain unescaped slashes. Use a URL-escaping library or replace slashes with %2F.{note}
...
Other valid properties may be inferred from [[reading the properties|#Read bucket information]], but are less-frequently changed.
...
Important headers:
* {{Accept}} \- When "multipart/mixed" is the preferred content-type, objects with siblings will return all siblings in single request. See [[#Get all siblings in one request]] example. See also RFC 2616 - [[Accept header definition|http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.1]].
...
Optional query parameters:
* {{r}} \- (read quorum) how many replicas need to agree when retrieving the object ([[default is defined by the bucket|#Set bucket properties]])
...
{note:title=Siblings}
{anchor:siblings}
When {{allow_mult}} is set to true in the [[bucket properties|#Set bucket properties]], concurrent updates are allowed to create "sibling" objects, meaning that the object has any number of different values that are related to one another by the vector clock.  This allows your application to use its own conflict resolution technique.
...
To resolve the conflict, [[store|#Store a new or existing object with a key]] the resolved version with the {{X-Riak-Vclock}} given in the response.
{note}
...
h4. Siblings examples

h5. Manually requesting siblings
{code}
...
Optional query parameters:
* {{rw}} \- quorum for both operations (get and put) involved in deleting an object. ([[default is defined by the bucket|#Set bucket properties]])
...
{info:title=Link specifications}
...
{note:title=Understanding the response body}
