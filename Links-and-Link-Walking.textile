...
{code}
$ curl -v -X PUT -H 'Link: </riak/people/dhh>; riaktag="friend"' -H "content-type: text/plain" http://127.0.0.1:8091/riak/people/timoreilly -d 'I am an excellent public speaker.'
{code}
...
{code}
$ curl -v -X PUT -H "content-type: text/plain" http://127.0.0.1:8091/riak/people/dhh -d 'I drive a Zonda.'
{code}
...
Once you have tagged objects in Riak with links, you can then traverse them with an operation called "Link Walking." 

To continue with the example from above, the "timoreilly" object is now pointer to the "dhh" object located in the "people" bucket. We can us a link walking query to follow the link from "timoreilly" to "dhh." Here is what that query would look like:

{code}
$ curl -v http://127.0.0.1:8093/riak/people/timoreilly/people,friend,1
{code}
...
{code}
$ curl -v http://127.0.0.1:8093/riak/people/timoreilly/_,friend,1
{code}
...
# Performed a link walking query to traverse the link from one object to another. 
...
h4. A Magnificent Link Walking Screen Cast
...
{widget:url=http://vimeo.com/14563219}
{widget}


h4. Link Walking Scripts
