...
{note:title=Configuration changes in 0.10}
...
{note:title=Configuration changes in 0.12}
...
Properties used for buckets that have not been explicitly defined (see also [[Setting bucket properties|REST API#Set bucket properties]]). Useful for setting default bucket behavior such as:
...
{code:language=erlang|title=default}
...
{note}The "ring_creation_size" should be established before your cluster is live and running and should not be changed thereafter.{note}

h4. target_n_val

"The highest {{n_val}} that I generally intend to use." Affects how partitions are distributed amongst the cluster and preflists calculated, helping ensure that data is never stored to the same physical node more than once. *You will only need to change this setting in rare circumstances.*
...
The base of the path in the URL exposing [[RIAK:MapReduce]] via HTTP. (default: {{"mapred"}})
...
Enables the statistics-aggregator (/stats URL and {{riak-admin status}} command) if set to {{true}}. (default: {{true}})
...
Specify the backend to use for a bucket with {{riak_client:set_bucket(BucketName,\[\{backend, BackendName\}\]}} in Erlang or {{\{"props":{"backend":"BackendName"\}\}}} in the [[REST API|REST API#Set bucket properties]].
...
{note:title= Dynamically Changing ttl}There is currently no way to dynamically change the ttl per bucket. The current work around would be to define multiple {{riak_cache_backends}} under {{riak_multi_backend}} with different ttl values.{note}
