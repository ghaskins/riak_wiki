...
Map/reduce functions can be written in Erlang. Erlang functions have complete access to the Riak Erlang API.

h4. Javascript

Riak also supports writing map/reduce functions in Javascript. Mozilla's Spidermonkey engine provides the runtime environment. Pre-defined Javascript functions run almost as fast as Erlang functions. Javascript functions are currently permitted read-only access to Riak.
