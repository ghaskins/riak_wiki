...
* *data\_root* - Set to the location where data files are written,relative to the Riak Search root directory.
...
* *max\_compact\_segments* - The maximum number of segments to compact during a compaction. Smaller values will to quicker compactions and a more balanced number of files in each partition, at the expense of more frequent compactions, and a higher likelihood of compacting the same data multiple times.
