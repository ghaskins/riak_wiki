h2. Webmachine request/response data
...
|@streambody()@|A webmachine [[streamed body format|Webmachine--Streamed-Body]]|
...
|@disp_path(rd()) -> string()@|The "local" path of the resource URI; the part after any prefix used in [[dispatch configuration|Webmachine---Request-Dispatching]]. Of the three path accessors, this is the one you usually want. This is also the one that will change after create_path is called in your [[resource|Webmachine---Resources]].|
...
|@path_info(atom(),rd()) -> 'undefined'@ &#124; @string()@|Looks up a binding as described in [[dispatch configuration|Webmachine---Request-Dispatching]].|
|@path_info(rd()) -> any()@|The dictionary of bindings as described in [[dispatch configuration|Webmachine---Request-Dispatching]].|
...
|@stream_req_body(rd(),integer()) -> streambody()@|The incoming request body in [[streamed|Webmachine---Streamed-Body]] form, with hunks no bigger than the integer argument.|
...
|@set_resp_body(streambody(),rd()) -> rd()@|Use this [[streamed body|Webmachine---Streamed-Body]] to produce the outgoing response body on demand.|
