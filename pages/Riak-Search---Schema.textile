...
* *FIELDNAME\_num* - Numeric field. Uses Integer analyzer. Values are padded to 10 characters.
* *FIELDNAME\_int* - Numeric field. Uses Integer analyzer. Values are padded to 10 characters.
* *FIELDNAME\_dt* - Date field. Uses No-Op analyzer.
* *FIELDNAME\_date* - Date field. Uses No-Op analyzer.
* *FIELDNAME\_txt* - Full text field. Uses Standard Analyzer.
* *FIELDNAME\_text* - Full text field. Uses Standard Analyzer.
...
h4. Whitespace Analyzer Factory
...
For example, the text "It's well-known fact that a picture is worth 1000 words." is split into the following tokens: ["It's", "a", "well-known", "fact", "that", "a", "picture", "is", "worth", "1000", "words."]. Notice that capitalization and punctiation is preserved.
...
The Standard Analyzer and the Whitespace Analyzer have Java versions based on Lucene's StandardTokenizer and WhitespaceTokenizer. 

To use the Java analyzers, you will first need to tell the system to launch a Java Virtual Machine during when Riak starts. This is done by setting the 'analysis_port' setting in the 'qilr' section of the app.config:

```
{analysis_port, 6095 },
```

Then, set the *analyzer_factory* setting as seen below:

```
%% For the Whitespace Analyzer...
...
The main advantage of using Java-based analyzers is better support for non-English text, and non-ASCII characters.

h4. Custom Analyzers

You can create your own custom analyzers in Erlang or Java. 

Some tips:

* Model your custom analyzer after an existing analyzer. 
  See "apps/qilr/src/text_analyzers.erl" for sample Erlang code.
  See "apps/qilr/java_src/analysis_server/src/com/bash/search/analysis/WhitespaceAnalyzerFactory.java" for sample Java code.
 
* The analyzer should take a string and configuration parameters and
  return a list of tokens. The order of tokens is important for
  proximity searching.
  
* Make sure to put your compiled analyzer on the Erlang code path or
