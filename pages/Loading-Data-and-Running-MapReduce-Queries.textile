...
<dl>
<dt>[[goog.csv|/attachments/goog.csv]]</dt>
<dd>Google historical stock data.</dd>
<dt>[[load_stocks.rb|/attachments/load_stocks.rb]]</dt>
<dd>Alternative script in Ruby to load the data.</dd>
<dt>[[load_data|/attachments/load_data]]</dt>
<dd>Erlang script to load data (as shown in snippet).</dd>
</dl>
...
h3. Why do we use MapReduce for Querying Riak?
...
After pressing return, paste your job in, for example the one shown below in the section "Complete Job", press return again, and then Ctrl-D to submit it. This way of running MapReduce queries is not
specific to this tutorial, but it comes in very handy to just run quick fire-and-forget queries from the command line in general. With a client library, most of the dirty work of assembling the JSON
that's sent to Riak will be done for you.</div>

h3. Map: find the days where the high was over $600.00
...
h3. Map: find the days where the close is lower than open
...
h3. Map and Reduce: find the maximum daily variance in price by month
...
<div class="info"><div class="title">Additional Reading for this Section</div>* [[MapReduce In Depth|MapReduce]]
...
* [[Google's Original MapReduce Paper|http://labs.google.com/papers/mapreduce.html]]</div>
