...
"RAM":http://en.wikipedia.org/wiki/Random-access_memory should be viewed as the most import resource when sizing your Riak cluster. Aside from helping you keep more data closer to your users, memory will also be required when running complex MapReduce queries, and caching data to provide low latency request times.
...
<div class="info">
# _static bitcask per key overhead_ - 40 bytes per key
...
the amount of RAM you would need for Bitcask is about *32.6 GBs*.
...
Now that you have a an idea of how much RAM you'll need, it's time to think about disk space. Disk space needs are much easier to calculate and essentially boil down to this simple equation:
...
Ring size is the number of partitions that make up you Riak Cluster. This is a number that is configured before you cluster is started, and is set in your app.config file under the [[ring_creation_size|Configuration Files#ring_creation_size]] parameter. 
