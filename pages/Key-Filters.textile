...
Key filters can be thought of as a series or pipeline of "transformations":#Transform-functions and "predicates":#Predicate-functions that attempt match keys produced by the list-keys operation.  Keys that match the predicates are fed into the MapReduce query as if they had been specified manually.
...
h2(#examples). Example query solutions
