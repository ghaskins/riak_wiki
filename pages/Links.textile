...
The essential way to read and modify links via the [[HTTP API]] is the [[HTTP Link header|http://www.martinpayne.me.uk/experiments/HTTP/Link-Header/HTTP-Link-example.html]].  This header emulates the purpose of &lt;link&gt; tags in HTML, that is, establishing relationships to other HTTP resources.  The format that Riak uses is like so:
