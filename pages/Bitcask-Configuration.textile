...
Bitcask is the default storage backend for Riak and its default settings work for many use-cases. In some scenarios, however, you might want to adjust the configuration to suit your production needs. Some of those scenarios are:
...
The @open_timeout@ setting specifies how long Bitcask will block on startup while attempting to create or open the data directory. The value is in seconds and the default is @4@. You generally need not change this value.
...
By default, Bitcask keeps all of your data around.  If your data has limited time-value, or if simply for space reasons you need to purge data, you can set the @expiry_secs@ option. This option is set to @-1@ by default, which disables automatic expiration.  If you needed to purge data automatically after 1 day, set the value to @86400@.
