...
```bash
$ curl -X PUT -H "Content-Type: application/json" -d '{"props":{"n_val":5}}' \
  http://riak-host:8098/riak/bucket
```


Changing the N value after a bucket has data in it is _not recommended_.  If you do change the value, especially increasing it, you might need to force read repair. Overwritten objects and newly stored objects will automatically be replicated to the correct number of nodes.

h2. Read Repair

[[Read repair|Riak Glossary]] occurs when a successful read occurs -- that is, the quorum was met -- but not all replicas from which the object was requested agreed on the value.  There are two possibilities here for the errant nodes:
