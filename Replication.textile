...
The N value also affects the behavior of read ([[GET|REST API#Read object]]) and write ([[PUT|REST API#Store a new or existing object with a key]]) requests. The tunable parameters you can submit with requests are bound by the N value. For example, if {{N=3}}, the maximum read quorum ({{R}}) you can request is also 3.  If some nodes containing the data you are requesting are down, an {{R}} value larger than the number of available nodes with the data will cause the read to fail.

h2. Setting the N value ({{n_val}})
...
Changing the N value after a bucket has data in it is _not recommended_.  If you do change the value, especially increasing it, you might need to [[force read repair|#Forcing Read Repair]]. Overwritten objects and newly stored objects will automatically be replicated to the correct number of nodes.

h2. Read Repair

[[Read repair|Riak Glossary#Read Repair]] occurs when a successful read occurs -- that is, the quorum was met -- but not all replicas from which the object was requested agreed on the value.  There are two possibilities here for the errant nodes:

# The node responded with a "not found" for the object, meaning it doesn't have a copy.
# The node responded with a [[vector clock|Riak Glossary#Vector Clock]] that is an ancestor of the vector clock of the successful read.
...
h2. So what does N=3 really mean?
...
As nodes are added and removed from the cluster, the ownership of partitions changes and _may_ result in an uneven distribution of the data.  On some rare occasions, Riak will also aggressively reshuffle ownership of the partitions to achieve a more even balance.
...
h2. Understanding replication by example
...
h3. Routing an object to a set of partitions

* Assume we have 3 nodes
* Assume we store 3 replicas per object (N=3)
* Assume we have 8 partitions (ring_creation_size=8) - _it is not recommended to use such a small ring size; this is for demonstration purposes only_
...
h3. Processing partition requests
